...
from sherlock_holmes.domain.entities.evidence import EvidenceRecord
...
def compare_records(
    *,
    ...
    comparable = [
        c for c in comparisons if c.status not in {"unresolved", "missing_in_manual", "missing_in_official"}
    ]
    ...
    critical_divergent = any(c.field_name in _CRITICAL_FIELDS and c.status == "divergent" for c in comparable)
